...
class IOStat(object):
    def __init__(self, path='/usr/bin/iostat',disks=[], no_dm_name=False):
    #def __init__(self, path='/usr/bin/iostat', interval=1, count=1, disks=[], no_dm_name=False):
        self.path = path
    ...
    def parse_diskstats(self, input):
        ...
        for d in ds:
            if d:
                d = d.split()
                d = [re.sub(br',',b'.',element) for element in d]
                dev = d.pop(0)
