...
class MigrationWorker:
    ...
    def run(self):
        ...
        for x in range(self.TOTAL_STREAM_COUNT):
            ...
            #        continue
            #else:
            #    return self.returned_data_creator(rpc_client.request_migration.__name__, code=code)
    ...
    def send_checkpoint(self):
        c = self._cli.container_presence(self._c_name)
        cp_path = '{0}/{1}/'.format(self._config['checkpoint']['default_cp_dir'], c.id)
        return Rsync.call(cp_path, cp_path, 'miura', src_addr=None, dst_addr=self._dst_addr)
